...
def Func_Filter_Phantom(Phantom, param):
    ...
    temp = (my * my) / (2. * GstdY * GstdY)
    temp = temp + (mx * mx) / (2. * GstdX * GstdX)
    temp = temp + (mz * mz) / (2. * GstdZ * GstdZ)
    del mx, my, mz
    h = np.exp(-temp)
    del temp
    # h = np.exp(-(my * my) / (2. * GstdY * GstdY)-(mx * mx) / (2. * GstdX * GstdX)-(mz * mz) / (2. * GstdZ * GstdZ))
...
def RLReconstructionFourier3_SingleAngle(param, Phantom, PSF):
    ...
    Image_stack[Image_stack < param.BGThreshold] = param.threshold
    Image_stack = np.maximum(Image_stack - param.BackGround, param.threshold)
    ...
    ErrorRate_Iter = 1
    SingleAngleIter = 1
    for i_iter in range(0, SingleAngleIter):
        for i_Map in range(0, map.shape[0]):
            ...
            ErrorRate_Iter = (ErrorBack * param.StepRate + (1 - param.StepRate)) * ErrorRate_Iter
    ...
    if param.CompressRate == 1:
        ErrorRate_Iter = ErrorRate_Iter
    else:
        ErrorRate_Iter, param = Func_DeZCP_ErrorBack(ErrorRate_Iter, param)

    Phantom = ErrorRate_Iter * Phantom
    del ErrorRate_Iter
    torch.cuda.empty_cache()
...
def RotationAberrationRLReconstructionFourier3_SingleAngle(param_RLRecon, param_aberration, param_ShiftShiftMap, Phantom,
                                                           RefractionIndex, RayInit, PSF):
    """
    基于Richardson–Lucy的单角度优化流程 Richardson–Lucy-based single-angle optimization process
    ...
    # Reconstruction
    [param_RLRecon, warped_Roted_Phantom] = RLReconstructionFourier3_SingleAngle(param_RLRecon, warped_Roted_Phantom, PSF) # 16530mb




    # Edge Cut
    warped_Roted_Phantom = EdgeCut(warped_Roted_Phantom, param_RLRecon)

    # Gfilter
    warped_Roted_Phantom, _ = Func_Filter_Phantom(warped_Roted_Phantom, param_RLRecon)
    ...
    if param_RLRecon.FlagAberration:
        ...
        warped_Roted_Phantom_ = warped_Roted_Phantom.reshape(1, 1, PhantomSize[0], PhantomSize[1], PhantomSize[2])
        Roted_Phantom = F.grid_sample(warped_Roted_Phantom_, pathsmesh_interp_unwarp,
                                               mode='bilinear', padding_mode='zeros', align_corners=True)
        del warped_Roted_Phantom_, pathsmesh_interp_unwarp
        torch.cuda.empty_cache()  #  5500mb
        Roted_Phantom = Roted_Phantom.squeeze()
    else:
        Roted_Phantom = warped_Roted_Phantom

    # deRotation
    Roted_Phantom_forAVG = Roted_Phantom
    Roted_Phantom_forAVG = torch.from_numpy(param_RLRecon.AvgMusk).to(param_RLRecon.device) * Roted_Phantom_forAVG
    ...
    warped_Phantom = warped_Roted_Phantom
    Phantom = Roted_Phantom
    Phantom_forAVG = Roted_Phantom_forAVG

    if param_RLRecon.thetaX != 0:
        warped_Phantom = Rotation_X(warped_Phantom, param_RLRecon)
        Phantom = Rotation_X(Phantom, param_RLRecon)
        Phantom_forAVG = Rotation_X(Phantom_forAVG, param_RLRecon)
    if param_RLRecon.thetaY != 0:
        warped_Phantom = Rotation_X(warped_Phantom, param_RLRecon)
        Phantom = Rotation_Y(Phantom, param_RLRecon)
        Phantom_forAVG = Rotation_Y(Phantom_forAVG, param_RLRecon)
    if param_RLRecon.thetaZ != 0:
        warped_Phantom = Rotation_X(warped_Phantom, param_RLRecon)
        Phantom = Rotation_Z(Phantom, param_RLRecon)
        Phantom_forAVG = Rotation_Z(Phantom_forAVG, param_RLRecon)


    torch.cuda.empty_cache() # 2235 mb

    return Phantom, Phantom_forAVG, warped_Phantom
    # return warped_Phantom



def RotationAberrationRLReconstructionFourier3(param_RLRecon, param_ShiftShiftMap, Phantom,
                                               RefractionIndex, RayInitList, PSF):
    """
    基于Richardson–Lucy的多角度优化流程 Richardson–Lucy-based multi-angle optimization process
    :param param_RLRecon: Richardson–Lucy重建参数 Richardson–Lucy reconstruction parameters
    :param param_ShiftShiftMap: 反向像差参数 Reverse aberration parameters
    :param Phantom: 强度矩阵 intensity Matrix
    :param RefractionIndex: 折射率矩阵（本项目未使用）Refraction Index Matrix(Not used in this project)
    ...
    Phantom_List = torch.zeros(RotAngle, PhantomSize[0], PhantomSize[1], PhantomSize[2])
    warped_Phantom_List = torch.zeros(RotAngle, PhantomSize[0], PhantomSize[1], PhantomSize[2])
    ...
    for epoch in range(0, EpochNum):
        ...
        for i_RotAngle in range(0, RotAngle):
            ...
            Phantom, Phantom_forAVG, warped_Phantom = RotationAberrationRLReconstructionFourier3_SingleAngle(
                param_RLRecon, param_aberration, param_ShiftShiftMap, Phantom,
                RefractionIndex, RayInitList[i_RotAngle, :, :, :], PSF)

            torch.cuda.empty_cache()
            warped_Phantom_List[i_RotAngle, :, :, :] = warped_Phantom.cpu().detach()
            Phantom_List[i_RotAngle, :, :, :] = Phantom.cpu().detach()
            Phantom_AVG = Phantom_forAVG / RotAngle + Phantom_AVG
            torch.cuda.empty_cache()

            # Save
            addr = param_RLRecon.SavePath + 'Phantom_' + str(i_RotAngle)
            TiffStackSave(Phantom, addr)

            if param_RLRecon.FlagTestSave:
                addr = param_RLRecon.SavePathTest + 'Phantom_' + str(param_RLRecon.BigEpoch) + '_' + str(epoch) + '_' + str(i_RotAngle)
                TiffStackSave(Phantom, addr)

                addr = param_RLRecon.SavePathTest + 'Phantom_forAVG_' + str(param_RLRecon.BigEpoch) + '_' + str(epoch) + '_' + str(i_RotAngle)
                TiffStackSave(Phantom_forAVG, addr)

        addr = param_RLRecon.SavePath + 'Phantom_AVG_' + str(epoch)
        TiffStackSave(Phantom_AVG, addr)

    return Phantom_List, Phantom_AVG, warped_Phantom_List


def RotationAberrationBPReconstructionFourier3(param_RLRecon, param_ShiftShiftMap, Phantom,
                                               RefractionIndex, RayInitList, PSF):
    """
    基于back propregation的多角度优化流程 back-propregation-based multi-angle optimization process
    :param param_RLRecon: Richardson–Lucy重建参数 Richardson–Lucy reconstruction parameters
    :param param_ShiftShiftMap: 反向像差参数 Reverse aberration parameters
    :param Phantom: 强度矩阵 intensity Matrix
    :param RefractionIndex: 折射率矩阵（本项目未使用）Refraction Index Matrix(Not used in this project)
    ...
    EpochNum = param_RLRecon.EpochNum
    RotAngle = param_RLRecon.RotAngle
    Phantom_ = Phantom.to(param_RLRecon.device)
    RayInitList = RayInitList.to(param_RLRecon.device)
    ...
    for epoch in range(0, EpochNum):
        ...
        for i_RotAngle in range(0, RotAngle):
            ...
            torch.cuda.empty_cache()
            Phantom_List[i_RotAngle, :, :, :] = Phantom.cpu().detach()
            Phantom_AVG = Phantom_forAVG.cpu().detach() / RotAngle + Phantom_AVG
            torch.cuda.empty_cache()
            ...
            del Phantom, Phantom_forAVG, warped_Phantom

        addr = param_RLRecon.SavePath + 'Phantom_AVG_' + str(epoch)
        TiffStackSave(Phantom_AVG, addr)

    return Phantom_List, Phantom_AVG
